  ...
  const processPeriodData = (period: typeof sixWeekPeriods[0]) => {
    ...
    njhsMembers.forEach(memberName => {
      ...
      const memberLogs = logsInPeriod.filter(log => {
        ...
        // Special debugging for Dhriti Erusalagandi
        if (memberName === 'Dhriti Erusalagandi') {
          console.log(`  === SPECIAL DEBUG FOR DHRITI ===`);
          console.log(`  Checking log: ${log.first_name} ${log.last_name} (${log.user_email})`);
          console.log(`    Original names: firstName="${log.first_name}", lastName="${log.last_name}"`);
          console.log(`    Normalized: firstName="${logFirstName}", lastName="${logLastName}", combined="${combinedFullName}", emailPrefix="${logEmailPrefix}"`);
          console.log(`    Member normalized: "${memberNameNormalized}"`);
        } else {
          console.log(`  Checking log: ${log.first_name} ${log.last_name} (${log.user_email})`);
          console.log(`    Normalized: firstName="${logFirstName}", lastName="${logLastName}", combined="${combinedFullName}", emailPrefix="${logEmailPrefix}"`);
        }
        ...
        if (memberName === 'Dhriti Erusalagandi') {
          ...
          console.log(`    Comparing: "${combinedFullName}" === "${memberNameNormalized}" = ${match1}`);
          console.log(`    Email prefix: "${logEmailPrefix}".startsWith("${memberNameNormalized}") = ${match2}`);
        } else {
          console.log(`    Match results: fullName=${match1}, emailPrefix=${match2}, firstNameOnly=${match3}`);
        }
