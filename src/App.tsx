...
function Header() {
  ...
  const linkClass = (path: string) =>
    location.pathname === path
      ? 'font-bold text-primary-dark bg-accent rounded-xl px-6 py-2 shadow transition'
      : 'font-bold text-white text-fill-hover-yellow px-6 py-2 rounded-xl transition';
  return (
    <header className="sticky top-0 z-20 bg-gradient-to-r from-blue-700 via-blue-500 to-blue-300 shadow-lg">
      <nav className="max-w-5xl mx-auto flex items-center justify-between px-4 py-3">
        ...
        <div className="flex gap-4 items-center font-semibold">
          {user && <Link to="/log" className={linkClass('/log')}>Log Hours</Link>}
          ...
          {user && isSuperAdmin && <Link to="/admin" className={linkClass('/admin')}>Admin</Link>}
          {user ? (
            ...
            <Link to="/login" className={linkClass('/login') + ' ml-4'}>Sign in</Link>
          )}
        </div>
      </nav>
    </header>
